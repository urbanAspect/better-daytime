...
class Tile:
    ...
    # Edited function - required element latlon: LatLon from module LatLon
    # Module LatLon is written in python 2.7 which is not supported since 2020
    # Changed function to take 3 parameters
    # latitude and longitude have to be floats!!! - I dont want to fuck around more
    ...
    def get_elevation(self, latitude: float, longitude: float) -> float:
        ...
        Raises:
            NoElevationDataError: If the specified location is not contained within the
                DTED file.
        """
        # ---------This check was removed, works if parameters are type float ------------
        ...
        # latitude and longitude originally objects latlon.latitude and latlon.longitude
        latitude_index = round((latitude - origin_latitude) * (lat_count - 1))
        longitude_index = round((longitude - origin_longitude) * (lon_count - 1))
